...
export default function Gallery() {
  ...
  const [likes, setLikes] = useLocalStorageState<Record<string, boolean>>("gallery-likes", {});
  ...
  const toggleLike = useCallback(
    (src: string, e?: React.MouseEvent) => {
      e?.stopPropagation();
      setLikes({ ...likes, [src]: !likes[src] });
    },
    [likes, setLikes],
  );
  ...
  const goPrev = useCallback(() => {
    setLightbox((prev) =>
      prev ? { ...prev, index: (prev.index - 1 + prev.list.length) % prev.list.length } : null,
    );
  }, []);
        ...
        <div className="reveal mb-10 flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <h2 className="font-display text-5xl sm:text-6xl">
              Inside The <span className="text-gradient-gold">Gym</span>.
            </h2>
          ...
          <div className="inline-flex border border-border">
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 text-xs uppercase tracking-widest transition-colors ${
                tab === "photos"
            ...
            <button
              ...
              className={`flex items-center gap-2 border-l border-border px-5 py-2 text-xs uppercase tracking-widest transition-colors ${
                tab === "videos"
                  ? "bg-primary text-primary-foreground"
                  : "text-muted-foreground hover:text-[color:var(--gold)]"
              }`}
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`absolute bottom-2 right-2 z-10 flex h-8 w-8 items-center justify-center rounded-full border bg-background/70 text-sm backdrop-blur-sm transition-all hover:scale-110 ${
                        liked ? "border-red-500 text-red-500" : "border-white/40 text-white/70"
                      }`}
            ...
            {videos.map((n) => (
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="mt-4 font-display text-lg uppercase tracking-widest">
                    Video {String(n).padStart(2, "0")}
                  </span>
      ...
      {lightbox && (
        ...
        >
          ...
          <button
            ...
            className={`absolute right-4 top-16 z-10 flex h-10 w-10 items-center justify-center rounded-full border bg-black/50 text-sm backdrop-blur-sm transition-all hover:scale-110 ${
              likes[lightbox.list[lightbox.index]] ? "border-red-500 text-red-500" : "border-white/20 text-white/70"
            }`}
          >
            <Heart
              className={`h-5 w-5 ${likes[lightbox.list[lightbox.index]] ? "fill-red-500" : ""}`}
            />
          </button>
          ...
          {errored.has(lightbox.list[lightbox.index]) ? (
            ...
            // eslint-disable-next-line @next/next/no-img-element -- lightbox needs raw viewport sizing; images are user-triggered overlays, not LCP
            <img
