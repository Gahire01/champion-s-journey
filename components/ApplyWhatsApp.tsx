...
const input =
  "w-full border border-input bg-background px-4 py-3 text-sm outline-none placeholder:text-muted-foreground focus:border-[color:var(--gold)] transition-colors";
...
export default function ApplyWhatsApp() {
  ...
  return (
    <section
      id="apply"
      className="relative overflow-hidden border-y border-border bg-gradient-to-br from-[#25D366]/10 via-background to-primary/10 py-32"
    >
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-5">
        <div className="reveal lg:col-span-2">
          ...
          <h2 className="font-display text-5xl sm:text-6xl">
            One <span className="text-gradient-gold">Message</span>
            <br />
            Away.
          </h2>
          <p className="mt-4 max-w-md text-muted-foreground">
            Fill this out and we&apos;ll open WhatsApp with your application pre-filled. A coach replies
            within minutes during gym hours.
          </p>
        ...
        <form onSubmit={onSubmit} className="reveal border border-border bg-card p-8 lg:col-span-3">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            >
              {["Beginner", "Some experience", "Amateur", "Professional"].map((p) => (
                <option key={p}>{p}</option>
              ))}
            </select>
          </div>
          <textarea
            rows={4}
            placeholder="What's your goal? (fitness, competition, self-defense…)"
            className={`${input} mt-4`}
            value={form.goal}
